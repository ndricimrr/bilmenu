...
function parseDataMSO_07_2024(responseData) {
  ...
  if (MsoNormalTableElements.length === 2) {
    ...
    const fixMenuTRs = filteredFixMenuMSONormalTable
      .find("tbody > tr")
      .slice(2);
    ...
    fixMenuTRs.each((index, element) => {
      ...
      if (index % 12 === 0) {
        ...
        if (currentDay) {
          // skip adding to empty day on first iteration
      ...
      const currentDishTr = currentDishTrRaw
        .replace(/\s+/g, " ")
        .replace(/[\n\t]+/g, " ")
        .replace(/\bveya\b/g, "")
        .trim();
      const currentDishEnRaw = $(element).find("td:nth-child(2)").text();
      const currentDishEn = currentDishEnRaw
        .replace(/\s+/g, " ")
        .replace(/[\n\t]+/g, " ")
        .replace(/\bor\b/g, "")
        .trim();
    ...
    const filteredAlternativeMenuMSONormalTableTr = filteredAlternativeMSOTable
      .find("tbody > tr")
      .slice(1);
    ...
    filteredAlternativeMenuMSONormalTableTr.each((index, element) => {
      ...
      const turkishSelector =
        index % 12 === 0 ? "td:nth-child(2)" : "td:nth-child(1)";
      const englishSelector =
        index % 12 === 0 ? "td:nth-child(3)" : "td:nth-child(2)";
